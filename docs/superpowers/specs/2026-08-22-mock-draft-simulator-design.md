...
If the seed input is blank, activation creates a readable random seed and records it in state. The `mock draft` button remains visible and enabled after activation. Clicking it during an active draft discards the current history and immediately starts a new draft with the displayed pending settings. A typed seed is honored; a blank seed resolves to a fresh random value. The page exposes no stop or close control. Reloading or navigating to a URL without draft state is outside the in-panel interaction contract.

The settings area remains rendered after activation, and every input—including the seed and roster counts—remains editable. These fields form a pending next-draft configuration initialized from the active draft settings. Editing them never changes the active draft's simulation, history, board, replay behavior, or URL hash. Pressing `mock draft` validates and normalizes the complete pending configuration, replaces the active draft with a fresh simulation using those settings, and only then updates the URL hash. Loading a different active draft through initial navigation or hash navigation reinitializes the pending configuration from that draft; ordinary picks and historical edits do not overwrite in-progress form edits. The title and always-enabled action share a compact header row. General draft controls occupy the first field row. Roster-position counts occupy a dedicated single-line second row that scrolls horizontally instead of wrapping.
...
- always-visible two-row editable pending settings, including an editable seed, with a title-adjacent always-enabled restart action and a non-wrapping horizontally scrolling roster row;
- pending-setting edits that leave the active simulation and URL hash untouched until restart, plus active-draft navigation that reinitializes the pending form;
