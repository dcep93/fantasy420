        ...
        .map((t) => ({
          ...
          allMatchups: Object.entries(allWrapped)
            .flatMap(([year, oldWrapped]) =>
              Object.entries(oldWrapped.ffMatchups).map(
                ([weekNum, matchups]) => ({
                  year,
                  weekNum,
                  opponent: matchups
            ...
            .map((obj) => ({
              ...obj,
              myTotal: (
                allWrapped[obj.year].ffTeams[t.id].rosters[obj.weekNum]
                  ?.starting || []
              )
                .map(
                  (playerId) =>
                    allWrapped[obj.year].nflPlayers[playerId].scores[
                      obj.weekNum
                    ] || 0
                )
                .reduce((a, b) => a + b, 0),
        ...
        .map((obj) => (
          <div key={obj.t.id}>
            <div style={bubbleStyle}>
              <h1>: {obj.t.name}</h1>
              <h3>
                wins: {obj.wins}/{obj.allMatchups.length} ={" "}
                {obj.ratio.toFixed(4)} .. PF-PA:{" "}
                {obj.allMatchups
                  .map((m) => m.myTotal)
                  .reduce((a, b) => a + b, 0)
                  .toFixed(2)}
              ...
              <div>
                {Object.entries(
                  groupByF(obj.allMatchups, (m) => m.opponent)
                ).map(([oppId, ms]) => (
                  <div
                    key={oppId}
                    title={ms
                      ...
                      .join("\n")}
                  >
                    {ms.filter((m) => m.myTotal > m.oppTotal).length}/
                    {ms.length} ..{" "}
                    {ms
                      .map((m) => m.myTotal)
                      .reduce((a, b) => a + b, 0)
