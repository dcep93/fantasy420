...
import { parseScoreboard, Snapshot } from "./data";
    ...
    inFlight = Promise.resolve().then(async () => {
      ...
      try {
        ...
        try {
          ...
          throw new Error("Install or reload the Fantasy420 Chrome extension (version 0.0.3 or later), then reload this page and your ESPN league tab.");
        } finally {
          clearTimeout(timer);
        }
        if (!response || ![0, 1].includes(response.fetched)) {
          update({ extensionAvailable: false, snapshot: null });
          throw new Error("Update the Fantasy420 Chrome extension to version 0.0.3 or later, then reload this page and your ESPN league tab.");
        }
