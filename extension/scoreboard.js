...
(() => {
  ...
  function allowedSender(sender) {
    try {
      ...
      return url.origin === "https://fantasy420.web.app" ||
  ...
  chrome.runtime.onMessageExternal.addListener((message, sender, reply) => {
    ...
    if (!allowedSender(sender)) {
      reply(fail("Scoreboard requests must come from Fantasy420."));
      return false;
    }
    ...
    (async () => {
      ...
      try {
        ...
        return fail(request.fetched
          ...
          : "Reload your ESPN league tab to activate the updated Fantasy420 extension, then try again.", request.fetched);
      } finally {
        clearTimeout(timer);
        pending.delete(requestId);
      }
    })().then(reply, () => reply(fail("Unable to find your ESPN league tab. Reload the Fantasy420 extension and try again.")));
    return true;
  });
})();
